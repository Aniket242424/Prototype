"""
Backtest runner for the End-of-Day Momentum strategy — branch R&D.

Same args as run.py but plugs EndOfDayMomentumStrategy into the engine.

Usage:
    python -m trading_agent.backtesting.run_eod \\
        --underlying NIFTY \\
        --start 2026-04-01 \\
        --end 2026-04-30
"""
...
@click.option(
    "--output-dir",
    type=click.Path(),
    default=str(REPO_ROOT / "reports" / "backtests_eod"),
)
@click.option("--force-refresh", is_flag=True)
def main(underlying, start, end, output_dir, force_refresh):
    """Run an EOD-Momentum backtest."""
    configure_logging()
    asyncio.run(_run(
...
async def _run(underlying, start_date, end_date, output_dir, force_refresh):
    instrument_key = INSTRUMENT_KEYS[underlying]
    log.info("backtest_eod.starting", underlying=underlying, start=str(start_date), end=str(end_date))

    fetcher = BarFetcher()
    bars = await fetcher.fetch_range(instrument_key, start_date, end_date, force_refresh)
    log.info("backtest_eod.bars_loaded", count=len(bars))
    if not bars:
        click.echo("No bars loaded.", err=True)
        return

    strategy = EndOfDayMomentumStrategy()
    engine = BacktestEngine(underlying=underlying, strategy=strategy)
    results = engine.run(bars)

    tag = f"{underlying}_EOD_{start_date}_{end_date}_{datetime.now().strftime('%H%M%S')}"
